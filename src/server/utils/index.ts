...
const getDbFromHarEntries = (entries: HarEntry[], _harEntryCallback?: HarMiddleware["_harEntryCallback"]) => {
...
const getDbFromKibanaHits = (hits: HIT[], _kibanaHitCallback?: KibanaMiddleware["_kibanaHitsCallback"]) => {
...
export const extractDbFromHAR = (
  ...
  try {
    ...
    if (!isHAR) return;
    const dbFromHar: UserTypes.Db = getDbFromHarEntries(entries, _harEntryCallback);
...
export const extractDbFromKibana = (
  ...
  try {
    ...
    if (!isKibana) return;
    const dbFromHits: UserTypes.Db = getDbFromKibanaHits(hits, _kibanaHitsCallback);
